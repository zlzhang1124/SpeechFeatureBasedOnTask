...
# 		      First version.
# @License  : None
# @Brief    : 提取VDD短片延时描述任务的特征: 继承自我介绍SI任务的特征
...
class VDDFeatures(SIFeatures):
    ...
    def __init__(self, input_f_audio: str, input_f_trans: str, f0min=75, f0max=600,
        ...
        """
        super(VDDFeatures, self).__init__(input_f_audio, input_f_trans, f0min, f0max, sil_thr, min_sil, min_snd)
    ...
    def get_all_feat(self, prefix='VDD_', use_=False):
        """
        获取当前所有特征
...
if __name__ == "__main__":
    ...
    audio_file = os.path.join(DATA_PATH_EXAMPLE, f"session_1/11_VDD/{sid}_vdd.wav")
    trans_file = os.path.join(TRANS_PATH_EXAMPLE, f"session_1/11_VDD/{sid}_vdd.txt")
    pd_feat = VDDFeatures(audio_file, trans_file).get_all_feat()
    print(pd_feat)
